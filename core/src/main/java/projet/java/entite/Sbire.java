...
public class Sbire implements Entite{
    ...
    // Coin spawn callback
    public interface CoinSpawnListener {
        void onCoinSpawn(float x, float y);
    }
    private CoinSpawnListener coinSpawnListener;

    public void setCoinSpawnListener(CoinSpawnListener listener) {
        this.coinSpawnListener = listener;
    }

    // Call this when the sbire dies to spawn a coin
    private void spawnCoinOnDeath() {
        if (coinSpawnListener != null) {
            coinSpawnListener.onCoinSpawn(this.positionX, this.positionY);
        }
    }
    ...
    public void mourir() {
        ...
        this.bouclier = 0;
        this.mana = 0;
        // Spawn a coin at the sbire's position
        spawnCoinOnDeath();
    }
